    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.scrollContainer}>
        <View style={styles.container}>
          ...
          <View style={styles.inputContainer}>
            {/* Input Field for Email (instead of phone number) */}
            <InputField
              iconName="envelope"   // Change the icon to envelope for email
              title="আপনার ইমেইল ঠিকানা লিখুন"
              placeholder="যেমন: example@mail.com"
              value={form.email}
              handleChangeText={(e) => setForm({ ...form, email: e })}
              keyboardType="email-address"   // Set email-specific keyboard
            />
